...
var load_collection = function(data) {
  ...
  fs.readdirSync(data.collection).forEach(function(file) {
    var filepath = path.resolve(dirpath, file);
    var data = fs.readFileSync(filepath, 'utf8');
    var slide = fm(data);
    slide.attributes.author =
    // TODO: check front matter is present and parsed ok
    slide.body = yaml.dump(slide.attributes) + '\n' + slide.body;
    // TODO: check required metadata is present
    var name = slide.attributes.name;
    if (collection[name]) {
      log.error('Multiple slides have the nane', name);
    } else {
